...
public class ItemPanelController : MonoBehaviour, ITabContent
{
    ...
    [SerializeField]
    private GameObject itemSlotPrefab; //리스트 대신 프리팹 1개

    [Tooltip("프리팹이 생성될 부모 Transform (Scroll View의 Content 오브젝트)")]
    [SerializeField]
    private Transform contentTransform; //슬롯이 생성될 위치

    //아이템 정보(Info) 창 UI 요소들
    ...
    //ItemSlotUI가 호출할 함수. 아이템 상세 정보를 오른쪽에 표시합니다.
    ...
    private void SetupSlotNavigation()
    {
        ...
        for (int i = 0; i < _spawnedSlots.Count; i++)
        {
            ...
            nav.selectOnRight = null; //오른쪽 정보창으로 포커스 이동을 원하면 여기를 수정
